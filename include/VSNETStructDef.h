...
typedef struct
{
    int                 m_en;       //中心OSD叠加使能开关 0:关闭 1:开启
}VSNETCENOSDPARAM;
...
typedef struct
{
    ...
    unsigned char               m_CaptureFlag;              //可抓拍标志，0-可抓拍  1-不可抓拍    
    unsigned char               m_TempMeasurementStatus;    //测温状态，0-可测温  1-不可测温  
    int                         m_FPA;                      //当前FPA X 100,单位C
    int                         m_FPASpeed;                 //当前FPA变化速率 X 1000，单位C/min；> 0升温，< 0降温
    VSNET_HUMANCALI_STATE_S     m_humancali_state;
    unsigned short              m_LastFFCTime;              //距上一次FFC的时间，单位S 
    short                       m_s16Reserve;
    int                         m_s32Reserve[2];//保留
...
typedef struct
{
  ...
  int       m_month; //月
  int       m_day;   //日
  int       m_D;     //1:定位无效; 2:2D定位; 3:3D定位;
  float     m_PDOP;  //精度因子 1~2理想 2~3极好 4~6好
  char      m_status;//状态, A=数据有效; V=数据无效
  char      m_mode;  //M=手动(强制操作在 2D 或 3D 模式),A=自动
  char      m_ew;    //东西方向(W=西, E=东)
  char      m_ns;    //南北方向( N=北, S=南)
  double    m_lat;   //纬度
  double    m_lng;   //经度
  double    m_speed; //速度 单位:Knot（节）
}VSNET_NMEA_GPSINFO;
...
typedef struct 
{
    int     datavalid;      //数据块标记 12表示有效
    double  ir_camera_matrix[9];//内参矩阵
    double  vis_camera_matrix[9];//内参矩阵
    double  ir_dist_coeff[8];//畸变参数
    double  vis_dist_coeff[8];//畸变参数
}DISTORT_PARAM;
...
typedef enum
{
    VSNET_DAYNIGHTSTATE_DAY = 0,     //白天
    VSNET_DAYNIGHTSTATE_NIGHT,       //黑夜
...
typedef struct
{
    ...
    int             m_Unregister;       //注册状态, 0:不在线, 1:已注册, 2:已注销
    int             m_EnableGB28181;    //接入使能, 1:enable, 0:disable
    char            m_videoId1[32];     //视频通道1ID
    char            m_videoId2[32];     //视频通道2ID
...
typedef struct
{
    unsigned short  m_onvif_port;       //端口号
    char            m_reserve[2];       //保留
} VSNET_ONVIFPORT_S;
...
{
    int m_mode;          //曝光模式，0 - 自动, 1 - 手动
    int m_exptime;       //曝光时间，VSNET_EXPTIME_EX_E，自动模式下，用作最大曝光时间; 手动模式下，用作手动曝光时间
    int m_maxExptime;    //最大曝光时间 [0x0, 0xFFFFFFFF]
    int m_s32Rev;
    unsigned int m_nAgain;        //固定模拟增益 [0x400, 0xFFFFFFFF]
    unsigned int m_nDgain;        //固定数字增益 [0x400, 0xFFFFFFFF]
    unsigned int m_nMaxAgain;     //最大数字增益 自动模式使用 [0x400, 0xFFFFFFFF]
    unsigned int m_nMaxDgain;     //最大数字增益 自动模式使用 [0x400, 0xFFFFFFFF]
    unsigned int m_u32Rev[8];
}VSNET_EXPOSUREPARAMEX_S; //曝光参数扩展
...
typedef struct
{
    ...
    char    m_reserved[32];       // 保留
}VSNETHANDHELDDEVPARAM;

typedef struct
{
    unsigned int m_calTime;        //最新校准时间, UTC时间，单位-秒
...
}VSNETSMARTTHRPARAM;  //可见光YUV在[l,h]范围的部分为红外测温区域
/*****AD96 param end *****/

/*iRay 宫格定制参数*/
#define GRIDS_RGN_MAX_NUM   64  //宫格数最多64, 相当于区域测温64个
typedef struct
{
    char    name[16];           /*rgn name*/
    char    osdType;            /*0:不叠加;1:最低温;2:最高温;3:平均温*/
    char    osdTrackTempEn;     /*温度点追踪叠加使能:0:不叠加;1:低温点追踪(不支持);2:高温点追踪*/
    char    reserve[6];
    float   overhtempthreshold; /*超最高温叠加阀值*/
}VSNETGRIDSATTRS;
...
typedef struct
{
    VSNETTEMPVALUE      m_region_temp[GRIDS_RGN_MAX_NUM]; //区域温度信息, 扩展区域数64
...
enum ALG_LEVEL_E{
	ALG_LEVEL_SHIELD 	= 0,    // 不检测模式
	ALG_LEVEL_SENSITIVE 	= 1,    // 管廊模式（见光报） ---极灵敏
	ALG_LEVEL_NORMAL 	= 2,    // 正常模式 
	ALG_LEVEL_OBTUSR 	= 3,    // 迟钝模式		
};

typedef struct  //区域  
{
	unsigned char			u8Vilad;					        //是否启用该区域 0 不启用    1启用
	unsigned char   		u8Level;					        //所属区域等级:			ALG_LEVEL_E
	unsigned char   		u8Rsvd[2];				        	//保留

	unsigned char 			u8Rect_x;			 		        //x坐标， [0-100]百分比 (*0.01)
	unsigned char   		u8Rect_y;					        //y坐标， [0-100]百分比 (*0.01)
	unsigned char  	 		u8Rect_w;					        //区域宽，[0-100]百分比 (*0.01)
	unsigned char   		u8Rect_h;					        //区域高，[0-100]百分比 (*0.01)

	//给温度使用
	short					s16LowTempThreshold;		        //低温预警阈值 温度预警阈值【*】实际值除以十，保留一位小数  例如：值为  1805  实际为  180.5 度
	short					s16HightTempThreshold;		        //高温预警阈值 温度预警阈值【*】实际值除以十，保留一位小数  例如：值为  1805  实际为  180.5 度
	unsigned short			u16TempRiseThreshold;	            //温升预警阈值  
	unsigned short  		u16TempRiseDelayTime;		        //温升预警持续时间（秒）
}PACKED_1 ALG_REGIONAL_S;

typedef struct // 火焰检测参数  
{
	unsigned char		u8Open;								//启用标识
	unsigned char 		u8FireLevel;						//FIREDETECT_LEVEL_E
	unsigned char 		u8Rsvd[34];							//保留
	unsigned short 		u16RowPixel;						//报警框竖直位移调整[-500,500]，默认值：0 【*】
	unsigned short 		u16ColPixel;						//报警框水平位移调整[-500,500]，默认值：0 【*】
	unsigned char 		u8VaryDegreeFire;					//火焰大小变化阈值[1,100]；默认值：20 【*】
	unsigned char 		u8VaryDegreeFirePos;				//火焰位置变化阈值[1,100]；默认值：20 【*】
	ALG_REGIONAL_S		stRegional[ALG_DETECT_RGN_MAX_NUM];	//9个区域
}PACKED_1 ALG_FIREPARAM_S;

typedef struct  //烟雾检测参数   
{
	unsigned char		u8SmokeMode;         				//感烟方式：0：不感烟；1：视觉感烟（默认值）；2、光束感烟 
	unsigned char		u8SmokeLevel;		          		//FIREDETECT_LEVEL_E 
	unsigned char		u8VaryDegreeSmoke;         			//烟雾大小变化阈值[1,100]，默认值：20 【*】
	unsigned char		u8VaryDegreeSmokePos;      			//烟雾位置变化阈值[1,100]，默认值：20 【*】
	unsigned char		u8Rsvd[8];				  			//保留
	ALG_REGIONAL_S		stRegional[ALG_DETECT_RGN_MAX_NUM];	 //反光板矩形框列表；设定反光板屏蔽框上限为9个 有标识该区域是否有效【*】
}PACKED_1 ALG_SMOGPARAM_S;

typedef struct //温度检测参数    
{ 
	unsigned char			u8Open;							    //是否启用		
	unsigned char			u8Rsvd[3];						    //保留				
	ALG_REGIONAL_S			stRegional[ALG_DETECT_RGN_MAX_NUM];	//9个区域			
}PACKED_1 ALG_TEMPPARAM_S;

typedef struct  // 扩展参数    
{ 
	unsigned char					u8PrivateNet;					//是否启用私有网络协议
	unsigned short					u16PrivatePort;					//默认 31111
	unsigned char 					u8Rsvd1;

	unsigned char					u8FireWater;					//是否启用水炮定位 
	unsigned char 					u8Rsvd2[23];
	unsigned short					u16FireWater_h;					//水炮水平视场角
	unsigned short					u16FireWater_t;					//水炮垂直视场角	

	unsigned char					u8Alarmhost;					//是否启用报警主机
	unsigned char					u8AlarmhostId;					//本地报警主机ID
	unsigned short					u16AlarmhostPort;				//
	char							szAlarmhostIp[32];				//服务器地址
}PACKED_1 ALG_EXTPARAM_S;

typedef struct//整个AI部分参数 
{
	ALG_FIREPARAM_S	stFireParam;	        //火焰检测参数
	ALG_SMOGPARAM_S	stSmogParam;	        //烟雾检测参数
	ALG_TEMPPARAM_S	stTempParam;	        //温度检测参数
	ALG_EXTPARAM_S	stExtParam;

	unsigned char   u8DrawLineFlag;			//是否划线 0:不划线  1:划线
	unsigned char   u8LensFlag;				//镜头焦距（鱼眼=>1,2.8mm=>2,4mm=>3,6mm=>4,8mm=>5,12mm=>6,16mm=>7,25mm=>8,
	                                        //即传入实参是：1,2,3,4,5,6,7,8，默认值为4（即6mm））【*】
	//OSD显示设置
	unsigned char   u8OSDAlarmPosition;		//(左上=>1,右上=>2,左下=>3,右下=>4，默认值：1) 【*】
	unsigned char 	u8OSDAlarmInfo;			//（显示框=>1,显示框和概率=>2,显示框和概率和坐标=>3,默认值：1）【*】

	unsigned char 	IsCheckCovered;			//true: 遮挡检测（默认值）； false: 不检测 【*】
	unsigned char 	IsCheckContaminated;	//true: 污染检测=遮挡检测（默认值）； false: 不检测 【*】
	unsigned char 	IsCheckRotated;			//true: 偏转检测（默认值）； false: 不检测 【*】
    unsigned char   IsCheckNet;             //true: 检测网络故障

	unsigned short 	u16AlarmAutoResetTime;	//报警自动复位时间[0,1800Secs]：报警后若干时间后自动复位；默认“0”时，不自动复位；否则，按设置时间选项自动复位; 
...
typedef struct
{
    unsigned short AlarmType;   /**<报警类型*/
    unsigned short AlarmSigVal; /**<报警信号*/
